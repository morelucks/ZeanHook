...
import {ZeanHook} from "../ZeanHook.sol";
...
abstract contract VolatilityManager is ZeanHookStorage {
    ...
    function _validateSwapSlippage(
        ...
        if (hookData.length >= 32) {
            ...
            if (userMaxSlippage < recommendedSlippage) {
                revert ZeanHook.UserSlippageBelowMinimum();
            }
            if (userMaxSlippage > MAX_SLIPPAGE) {
                revert ZeanHook.UserSlippageExceedsMaximum();
            }
        }
    }
