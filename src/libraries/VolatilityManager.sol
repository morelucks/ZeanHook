...
abstract contract VolatilityManager is ZeanHookStorage {
     ...
     */
    function calculateVolatilityAdjustedSlippage(PoolId poolId) public view returns (uint256 adjustedSlippage) {
     ...
     */
    function getVolatilityMetrics(PoolId poolId) external view returns (VolatilityMetrics memory metrics) {
     ...
     */
    function getPriceHistory(PoolId poolId, uint256 limit) external view returns (PriceData[] memory history) {
    ...
    function _recordPriceData(PoolId poolId, uint160 sqrtPriceX96) internal {
        ...
        history.push(PriceData({sqrtPriceX96: sqrtPriceX96, timestamp: block.timestamp}));
    ...
    function _validateSwapSlippage(uint256 recommendedSlippage, bytes calldata hookData) internal pure {
        if (hookData.length >= 128) {
            // 96 bytes for AVS proof + 32 bytes for slippage
            uint256 userMaxSlippage = abi.decode(hookData[96:], (uint256));
            require(userMaxSlippage >= recommendedSlippage, "User slippage below recommended minimum");
