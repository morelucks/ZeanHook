...
abstract contract CommitRevealManager is ZeanHookStorage {
    ...
    event CommitPhaseStarted(PoolId indexed poolId, uint256 startTime, uint256 endTime);

    event RevealPhaseStarted(PoolId indexed poolId, uint256 startTime, uint256 endTime);

    event InvalidReveal(PoolId indexed poolId, address indexed user, bytes32 indexed commitHash, string reason);

    // ========================= Modifiers =========================
    modifier onlyDuringCommitPhase(PoolId poolId) {
        require(batchStates[poolId].commitPhaseActive, "Not in commit phase");
        require(block.timestamp < batchStates[poolId].commitPhaseStart + COMMIT_PHASE_DURATION, "Commit phase ended");
        _;
    }

    modifier onlyDuringRevealPhase(PoolId poolId) {
        require(batchStates[poolId].revealPhaseActive, "Not in reveal phase");
        require(block.timestamp < batchStates[poolId].revealPhaseStart + REVEAL_PHASE_DURATION, "Reveal phase ended");
        _;
    }
    ...
    // ========================= External Functions =========================
    function commitSwap(PoolId poolId, bytes32 commitHash) external onlyDuringCommitPhase(poolId) {
        require(commitHash != bytes32(0), "Invalid commit hash");

        SwapCommit memory newCommit =
            SwapCommit({commitHash: commitHash, commitTime: block.timestamp, revealed: false, executed: false});
    ...
    function revealSwap(
        ...
        require(block.timestamp >= commit.commitTime + MIN_COMMIT_DELAY, "Reveal too early");

        // Verify commit hash
        bytes32 expectedHash = generateCommitHash(
            msg.sender, params, minAmountOut, maxSqrtPriceX96, minSqrtPriceX96, hookData, nonce, salt
        );
    ...
    function startCommitPhase(PoolId poolId) external onlyAuthorizedExecutor {
        ...
        emit CommitPhaseStarted(poolId, block.timestamp, block.timestamp + COMMIT_PHASE_DURATION);
    }
    ...
    function startRevealPhase(PoolId poolId) external onlyAuthorizedExecutor {
        ...
        require(block.timestamp >= batchState.commitPhaseStart + COMMIT_PHASE_DURATION, "Commit phase not ended");

        batchState.commitPhaseActive = false;
        batchState.revealPhaseActive = true;
        batchState.revealPhaseStart = block.timestamp;

        emit RevealPhaseStarted(poolId, block.timestamp, block.timestamp + REVEAL_PHASE_DURATION);
    }
    ...
    // ========================= View Functions =========================
    function getCommitPhaseInfo(PoolId poolId)
        external
        view
        returns (bool active, uint256 startTime, uint256 endTime, uint256 remainingTime)
    {
    ...
    function getRevealPhaseInfo(PoolId poolId)
        external
        view
        returns (bool active, uint256 startTime, uint256 endTime, uint256 remainingTime)
    {
