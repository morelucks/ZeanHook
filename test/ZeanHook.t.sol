...
contract ZeanHookTest is Test {
    ...
    function testBeforeSwap() public {
        ...
        bytes memory hookData = bytes.concat(new bytes(96), abi.encode(uint256(100))); // 1% slippage tolerance
    ...
    function testSwapWithInsufficientSlippage() public {
        ...
        bytes memory hookData = bytes.concat(new bytes(96), abi.encode(uint256(5))); // 0.05% slippage tolerance
    ...
    function testSwapWithExcessiveSlippage() public {
        ...
        bytes memory hookData = bytes.concat(new bytes(96), abi.encode(uint256(1000))); // 10% slippage tolerance
    ...
    function testRevealSwap() public {
        ...
        uint256 nonce = 12345;
        bytes32 salt = keccak256("test_salt");
        bytes32 commitHash = hook.generateCommitHash(user1, params, 0, 0, 0, "", nonce, salt);
